...
- **The cross-sell score counts product families, not products.** `cross_sold_products` can only name one of six banking families plus `other`, so insurance, credit cards, subscription perks and cashback all collapse into a single `other` and are counted once. Measured on the captured pages, ING's packs name 8 to 14 distinct add-ons and score 1/6, while pages naming 3 or 4 score 2/6 — the ranking is inverted for any bank whose cross-sell is mostly non-banking. Read the score as family coverage, never as how hard a page cross-sells.
