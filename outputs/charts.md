...
*Generated by `scripts/run_analysis.py` from `data\processed\campaigns_scored.csv` — 15 pages across 9 banks.*

---

## Why the charts use fewer features than the dictionary has

The dictionary defines 104 features. The comparisons here use **27**. Nothing is thrown away quietly — this is the whole reduction:
...
104 features in the dictionary
  ...
  = 27  features used in this comparison
```
...
**What this run shows.** ING scores **0.36** — traditional, but leaning towards the challengers. Computed over 27 features.

The two groups do not overlap: the most challenger-like incumbent sits at 0.36 and the most traditional challenger at 0.93, a gap of 0.57. That separation is what makes the axis meaningful — if the groups interleaved, the projection would be measuring noise.

| Bank | Category | Position |
| --- | --- | --- |
| bnp_paribas_fortis | traditional | -0.16 |
| argenta | traditional | -0.13 |
| belfius | traditional | -0.06 |
| crelan | traditional | -0.04 |
| kbc | traditional | 0.02 |
| ing **(focus)** | traditional | 0.36 |
| n26 | challenger | 0.93 |
| bunq | challenger | 0.98 |
| revolut | challenger | 1.09 |
...
**What this run shows.** The largest gap is **urgency marker count** at +2.5 SD (ing 3.50 against a peer mean of 1.10).

| Feature | Dimension | ing | Peer mean | Gap (SD) |
| --- | --- | --- | --- | --- |
| urgency_marker_count | tone_messaging | 3.50 | 1.10 | 2.46 |
| persuasion_lever_count | marketing_principles | 3.50 | 2.08 | 1.94 |
| fast_digital_onboarding_claim | banking_domain | 0.75 | 0.23 | 1.52 |
| background_luminance | colours_design | 0.20 | 0.59 | -1.17 |
| people_present | visuals | 1.00 | 0.62 | 0.77 |
| images_have_alt_text | banking_domain | 0.00 | 0.38 | -0.77 |
| rate_shown | topics_value_prop | 0.00 | 0.31 | -0.73 |
| cta_count | layout_structure | 5.50 | 14.48 | -0.73 |
...
**What this run shows.** The sharpest separator is **rate shown** (traditional 0.00 vs challenger 0.75, d = +3.12).

| Feature | Traditional | Challenger | Cohen's d |
| --- | --- | --- | --- |
| rate_shown | 0.00 | 0.75 | 3.12 |
| expat_cross_border_targeting | 0.18 | 1.00 | 2.31 |
| people_present | 0.91 | 0.25 | -1.84 |
| subscription_style_framing | 0.00 | 0.50 | 1.80 |
| question_count | 18.73 | 4.50 | -1.29 |
| hidden_conditions_behind_free_claim | 0.09 | 0.50 | 1.07 |
| institutional_trust_signal_present | 0.09 | 0.50 | 1.07 |
| cta_above_fold | 0.00 | 0.25 | 1.04 |
...
**What this run shows.** The banks closest to ING are **argenta** (5.4), **belfius** (5.7), **kbc** (6.1).

| Cluster | Banks |
| --- | --- |
| 1 | argenta, belfius, bnp_paribas_fortis, crelan, ing, kbc |
| 2 | bunq, n26, revolut |
...
| H3 | ING is the only traditional bank using animation | not supported | ing=0.75; other traditional banks above zero: ['crelan', 'kbc', 'belfius'] |
