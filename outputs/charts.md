...
*Generated by `scripts/run_analysis.py` from `data/processed/campaigns_scored.csv` — 12 pages across 10 banks.*

---

## Why the charts use fewer features than the dictionary has

The dictionary defines 104 features. The comparisons here use **29**. Nothing is thrown away quietly — this is the whole reduction:
...
104 features in the dictionary
  ...
  - 11  missing for some bank      cannot compare what one bank lacks
  -  7  no variation across banks  identical everywhere, carries no signal
  = 29  features used in this comparison
```
...
**What this run shows.** ING scores **0.18** — clearly with the traditional banks. Computed over 29 features.

The two groups do not overlap: the most challenger-like incumbent sits at 0.18 and the most traditional challenger at 0.98, a gap of 0.80. That separation is what makes the axis meaningful — if the groups interleaved, the projection would be measuring noise.

| Bank | Category | Position |
| --- | --- | --- |
| kbc | traditional | -0.15 |
| crelan | traditional | -0.13 |
| argenta | traditional | -0.11 |
| beobank | traditional | -0.07 |
| vdk | traditional | -0.00 |
| belfius | traditional | 0.11 |
| hellobank | traditional | 0.17 |
| ing **(focus)** | traditional | 0.18 |
| n26 | challenger | 0.98 |
| bunq | challenger | 1.02 |
...
**What this run shows.** The largest gap is **persuasion lever count** at +2.1 SD (ing 3.50 against a peer mean of 2.00).

| Feature | Dimension | ing | Peer mean | Gap (SD) |
| --- | --- | --- | --- | --- |
| persuasion_lever_count | marketing_principles | 3.50 | 2.00 | 2.12 |
| youth_student_targeting | banking_domain | 1.00 | 0.22 | 1.87 |
| numeric_claim_count | tone_messaging | 25.50 | 12.44 | 1.50 |
| value_prop_clarity | topics_value_prop | 5.00 | 4.40 | 1.22 |
| cta_count | layout_structure | 3.00 | 14.72 | -1.02 |
| hero_image_area_ratio | visuals | 0.53 | 0.29 | 0.90 |
| has_comparison_table | layout_structure | 0.00 | 0.44 | -0.89 |
| clarity_score | tone_messaging | 4.50 | 4.80 | -0.75 |

**What it cannot tell you.** With 9 peers the standard deviation is estimated from a handful of values, so a large gap on a feature where peers happen to agree closely is easy to overstate. Read the raw values in the table, not only the SD.
...
**This is descriptive, not a significance test.** Cohen's d is being used here purely as a *ranking device* for which features separate the groups most. With 8 bank(s) on one side and 2 on the other, no p-value would be meaningful, and none is computed anywhere in this project (PRD risk R-03).

**What this run shows.** The sharpest separator is **background luminance** (traditional 0.86 vs challenger 0.29, d = -4.18).

| Feature | Traditional | Challenger | Cohen's d |
| --- | --- | --- | --- |
| background_luminance | 0.86 | 0.29 | -4.18 |
| subscription_style_framing | 0.00 | 0.50 | 2.24 |
| rate_shown | 0.00 | 0.50 | 2.24 |
| first_time_investor_targeting | 0.00 | 0.50 | 2.24 |
| total_image_area_ratio | 0.11 | 0.38 | 2.10 |
| above_fold_element_count | 34.90 | 20.50 | -1.77 |
| page_height_px | 5,909 | 11,570 | 1.61 |
| hero_image_area_ratio | 0.26 | 0.63 | 1.58 |
...
**What this run shows.** The banks closest to ING are **belfius** (4.7), **vdk** (5.8), **hellobank** (6.0).

| Cluster | Banks |
| --- | --- |
| 1 | argenta, belfius, beobank, crelan, hellobank, ing, kbc, vdk |
| 2 | bunq, n26 |
