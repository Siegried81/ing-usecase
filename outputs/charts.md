...
*Generated by `scripts/run_analysis.py` from `data/processed/campaigns_scored.csv` — 5 pages across 4 banks.*

---

## Why the charts use fewer features than the dictionary has

The dictionary defines 101 features. The comparisons here use **39**. Nothing is thrown away quietly — this is the whole reduction:
...
101 features in the dictionary
  ...
  -  5  missing for some bank      cannot compare what one bank lacks
  - 10  no variation across banks  identical everywhere, carries no signal
  = 39  features used in this comparison
```
...
**What this run shows.** ING scores **0.00** — clearly with the traditional banks. Computed over 39 features.

The two groups do not overlap: the most challenger-like incumbent sits at 0.00 and the most traditional challenger at 0.99, a gap of 0.99. That separation is what makes the axis meaningful — if the groups interleaved, the projection would be measuring noise.

| Bank | Category | Position |
| --- | --- | --- |
| kbc | traditional | -0.00 |
| ing **(focus)** | traditional | 0.00 |
| n26 | challenger | 0.99 |
...
**What this run shows.** The largest gap is **numeric claim count** at +4.3 SD (ing 25.50 against a peer mean of 11.33).

| Feature | Dimension | ing | Peer mean | Gap (SD) |
| --- | --- | --- | --- | --- |
| numeric_claim_count | tone_messaging | 25.50 | 11.33 | 4.29 |
| sentence_count | tone_messaging | 163.00 | 83.33 | 3.80 |
| readability_score | tone_messaging | 53.80 | 34.77 | 3.55 |
| page_height_px | layout_structure | 12,088 | 6,131 | 3.38 |
| clarity_score | tone_messaging | 1.50 | 2.67 | -2.47 |
| value_prop_clarity | topics_value_prop | 1.50 | 2.67 | -2.47 |
| aida_coverage_score | marketing_principles | 0.00 | 3.00 | -2.12 |
| word_count | tone_messaging | 3,108 | 2,005 | 2.09 |
...
**What this run shows.** The sharpest separator is **aida coverage score** (traditional 0.33 vs challenger 4.00, d = +7.78).

| Feature | Traditional | Challenger | Cohen's d |
| --- | --- | --- | --- |
| aida_coverage_score | 0.33 | 4.00 | 7.78 |
| question_count | 18.33 | 4.00 | -3.71 |
| hero_image_area_ratio | 0.35 | 0.98 | 3.48 |
| second_person_ratio | 0.85 | 0.77 | -3.25 |
| clarity_score | 1.67 | 3.00 | 2.83 |
| value_prop_clarity | 1.67 | 3.00 | 2.83 |
| cta_count | 3.67 | 9.00 | 2.60 |
...
**What this run shows.** The banks closest to ING are **kbc** (9.9), **n26** (10.2), **revolut** (11.1).
