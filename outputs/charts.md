...
*Generated by `scripts/run_analysis.py` from `data/processed/campaigns.csv` — 6 pages across 5 banks.*

---

## Why the charts use fewer features than the dictionary has

The dictionary defines 101 features. The comparisons here use **35**. Nothing is thrown away quietly — this is the whole reduction:
...
101 features in the dictionary
  ...
  -  5  no variation across banks  identical everywhere, carries no signal
...
**What this run shows.** ING scores **-0.06** — clearly with the traditional banks. Computed over 35 features.

The two groups do not overlap: the most challenger-like incumbent sits at 0.23 and the most traditional challenger at 0.87, a gap of 0.64. That separation is what makes the axis meaningful — if the groups interleaved, the projection would be measuring noise.

| Bank | Category | Position |
| --- | --- | --- |
| kbc | traditional | -0.17 |
| ing **(focus)** | traditional | -0.06 |
| belfius | traditional | 0.23 |
| revolut | challenger | 0.87 |
| n26 | challenger | 1.13 |
...
**What this run shows.** The largest gap is **urgency marker count** at +4.0 SD (ing 2.00 against a peer mean of 0.25).

| Feature | Dimension | ing | Peer mean | Gap (SD) |
| --- | --- | --- | --- | --- |
| urgency_marker_count | tone_messaging | 2.00 | 0.25 | 4.04 |
| image_count | visuals | 41.00 | 14.50 | 3.14 |
| sentence_count | tone_messaging | 143.00 | 73.75 | 2.82 |
| cta_count | layout_structure | 3.00 | 8.00 | -2.24 |
| readability_score | tone_messaging | 44.80 | 32.83 | 2.09 |
| word_count | tone_messaging | 2,896 | 1,842 | 1.96 |
| expat_cross_border_targeting | banking_domain | 1.00 | 0.25 | 1.73 |
| page_height_px | layout_structure | 8,909 | 5,143 | 1.64 |
...
**What this run shows.** The sharpest separator is **hero image area ratio** (traditional 0.29 vs challenger 0.98, d = +4.08).

| Feature | Traditional | Challenger | Cohen's d |
| --- | --- | --- | --- |
| hero_image_area_ratio | 0.29 | 0.98 | 4.08 |
| cta_contrast_ratio | 6.73 | 16.73 | 2.86 |
| total_image_area_ratio | 0.12 | 0.41 | 2.14 |
| question_count | 16.00 | 4.00 | -2.14 |
| youth_student_targeting | 1.00 | 0.50 | -1.41 |
| institutional_trust_signal_present | 0.00 | 0.50 | 1.41 |
| mentions_loyalty_or_referral | 0.00 | 0.50 | 1.41 |
| hidden_conditions_behind_free_claim | 0.00 | 0.50 | 1.41 |
...
**What this run shows.** The banks closest to ING are **revolut** (9.4), **kbc** (9.9), **n26** (10.0).

| Cluster | Banks |
| --- | --- |
| 1 | belfius, kbc, n26, revolut |
| 2 | ing |
...
| --- | --- | --- | --- |
| H1 | Belfius is pretty verbose | not supported | belfius=1354.0; highest is ing=2896.5 |
| H2 | KBC is straight to the point | not supported | kbc=2644.0; lowest traditional is belfius=1354.0 |
| H3 | ING is the only traditional bank using animation | not supported | ing=0.50; other traditional banks above zero: ['kbc', 'belfius'] |
