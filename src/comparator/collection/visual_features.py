...
def extract_colours_from_image(
    ...
    `first_screen_px` measures background_luminance on the top crop rather
    than the whole page strip. The mean is pixel-weighted, so on a long page
    it reports how much white body copy the page has instead of how dark it
    looks: a black hero over a long white body scored 0.916 on a 14,516px
    capture whose first screen reads 0.521. Only luminance uses the crop -
    brand_colour_share deliberately scans the FULL page, because a brand
    accent is a few percent of a page and cropping would hide it again.
